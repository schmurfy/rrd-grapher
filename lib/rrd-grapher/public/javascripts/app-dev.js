    ...
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function setRange(axis) {
            var opts = axis.options,
                min = +(opts.min != null ? Math.min(opts.min, axis.datamin) : axis.datamin),
                max = +(opts.max != null ? Math.max(opts.max, axis.datamax) : axis.datamax),
                delta = max - min;
        ...
        return $.ajax({
          ...
          error: function(xhr, errText, err) {
            if ((xhr.status === 404) && console) {
              return console.log("URL No found: " + url);
            }
          }
        });
      });
...
(function() {
  ...
  window.CollectdNetworkStats = (function() {
    __extends(CollectdNetworkStats, GraphDefinition);
    function CollectdNetworkStats(container, host, can_send, ymin) {
      if (can_send == null) {
        can_send = true;
      }
      if (ymin == null) {
      ...
      CollectdNetworkStats.__super__.constructor.call(this, host, container, "Network Stats (collectd)", [Format.identity, Format.identity], ymin);
      this.graph.addSerie("" + this.host + "/network/queue_length", "value", "Queue length");
      this.graph.addSerie("" + this.host + "/network/total_values-dispatch-accepted", "value", "Values dipatch accepted", 2);
      this.graph.addSerie("" + this.host + "/network/total_values-dispatch-rejected", "value", "Values dipatch rejected", 2);
      if (can_send) {
        this.graph.addSerie("" + this.host + "/network/total_values-send-accepted", "value", "Values send accepted", 2);
        this.graph.addSerie("" + this.host + "/network/total_values-send-rejected", "value", "Values send rejected", 2);
      }
    }
    return CollectdNetworkStats;
  })();
  window.CollectdNetworkBandwidth = (function() {
    __extends(CollectdNetworkBandwidth, GraphDefinition);
    function CollectdNetworkBandwidth(container, host, can_send, ymin) {
      if (can_send == null) {
        can_send = true;
      }
      if (ymin == null) {
      ...
      CollectdNetworkBandwidth.__super__.constructor.call(this, host, container, "Network Usage (collectd)", [Format.size, Format.identity], ymin);
      this.graph.addSerie("" + this.host + "/network/if_octets", "rx", "Bytes Received");
      if (can_send) {
        this.graph.addSerie("" + this.host + "/network/if_octets", "tx", "Bytes Sent");
      }
      this.graph.addSerie("" + this.host + "/network/if_packets", "rx", "Packets Received", 2);
      if (can_send) {
        this.graph.addSerie("" + this.host + "/network/if_packets", "tx", "Packets Sent", 2);
      }
    }
    return CollectdNetworkBandwidth;
  })();

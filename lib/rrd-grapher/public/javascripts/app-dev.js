...
(function($) {
    function Plot(placeholder, data_, options_, plugins) {
        ...
        function setRange(axis) {
            ...
            if((opts.max != null) && (axis.datamax > opts.max)){
              max = axis.datamax;
            }
...
(function() {
  ...
  window.Graph = (function() {
    ...
    colors = ["#edc240", "#afd8f8", "#cb4b4b", "#4da74d", "#9440ed"];
    next_color = 0;
    function Graph(title, parent_container, formatters, limits) {
      this.formatters = formatters || [Format.identity, Format.identity];
      this.limits = limits || [[null, null], [null, null]];
      this.maxrows = 400;
      this.legend_containers = [];
      this.master_container = $("<div>").addClass("graph_container").appendTo(parent_container);
      this.graph_title = $("<div>").addClass("graph_title").appendTo(this.master_container);
      this.legend_containers[0] = $("<div>").addClass('legend').appendTo(this.master_container);
      $("<h3>").text("Legend").appendTo(this.legend_containers[0]);
      $("<div>").text("Legend").appendTo(this.legend_containers[0]);
      this.container = $("<div>").addClass("graph").appendTo(this.master_container);
      this.graph_title.text(title);
      this.series = [];
      this.lines = [];
      this.tooltip_point = null;
      this.to = Math.floor((new Date().getTime() / 1000) - 20);
        ...
        yaxes: [
          {
            min: this.limits[0][0],
            max: this.limits[0][1],
            tickFormatter: this.formatters[0],
            labelWidth: 100
          }, {
            min: this.limits[1][0],
            max: this.limits[1][1],
            ...
            labelWidth: 100,
            reserveSpace: true,
            tickFormatter: this.formatters[1]
          }
        ]
      };
    }
    ...
    Graph.prototype._build_query = function(s) {
      var query;
      query = "/rrd/" + s.rrd_path + "/values/" + this.from + "/" + this.to + "?maxrows=" + this.maxrows + "&ds_name=" + s.ds_name;
      if (s.rra) {
        query += "&rra=" + s.rra;
      }
      return query;
    ...
    Graph.prototype.update_graph = function(first) {
      ...
      first = first || false;
      urls = [];
      urls = $(this.series).select(function(s) {
        return s.enabled;
    ...
    Graph.prototype._update_graph_common = function(first, data_array) {
      ...
      $.each(this.lines, function(i, line) {
        return data_array.push(line.get_definition(this.from, this.to));
      });
      if (first) {
        this.plot = $.plot(this.container, data_array, this.flot_options);
        $(this.container).bind("plothover", __bind(function(event, pos, item) {
          return this.show_tooltip(event, pos, item);
        }, this));
        $(this.container).bind("plotselected", __bind(function(event, ranges) {
          this.set_interval(Time.client_to_server(ranges.xaxis.from), Time.client_to_server(ranges.xaxis.to));
          this.update_graph();
          return this.plot.clearSelection();
        }, this));
      } else {
      ...
      for (i = 0, _ref = this.series.length; 0 <= _ref ? i < _ref : i > _ref; 0 <= _ref ? i++ : i--) {
    ...
    Graph.prototype.show_tooltip = function(event, pos, item) {
      ...
      if (item) {
        ...
        if (this.tooltip_point !== id) {
          ...
          s = $(this.series).filter(function(i, s) {
        ...
        $("#tooltip").remove();
        return this.tooltip_point = null;
      }
    };
    Graph.prototype.create_legend = function(index) {
      ...
      this.legend_containers[index].find("div").html(table);
      return this.legend_containers[index].find(".legendColorBox").click(__bind(function(event) {
        var id, target;
...
(function() {
  ...
  window.GraphPage = (function() {
    function GraphPage(container, host) {
    ...
    GraphPage.prototype.refresh = function(interval, time) {
      var from, int, me, to;
      if (time == null) {
        time = null;
      }
      ...
      from = to - interval;
      $.each(this.graphs, function(i, g) {
        return g.set_interval(from, to);
      });
      if (time) {
  ...
  window.GraphDefinition = (function() {
    function GraphDefinition(host, container, title, formatters, ymin) {
      ...
      this.graph = new Graph(this.title, this.container, formatters, limits);
    }
...
(function() {
  window.StaticLine = (function() {
    ...
    StaticLine.prototype.get_definition = function(from, to) {
      return {
        data: [[this.convert_time(from), this.yvalue], [this.convert_time(to), this.yvalue]],
        legend: "",
        color: this.color
      };
    };
    StaticLine.prototype.convert_time = function(t) {
...
(function() {
  ...
  window.Time = {
    current_tz_to_utc: function(timestamp) {
      return parseInt(timestamp, 10) - tz_offset;
    },
    utc_to_current: function(timestamp) {
      return parseInt(timestamp, 10) + tz_offset;
    },
    server_to_client: function(timestamp) {
      return this.utc_to_current(timestamp) * 1000;
    },
    client_to_server: function(timestamp) {
      return Math.floor(this.current_tz_to_utc(timestamp / 1000));
    }
  };
}).call(this);
